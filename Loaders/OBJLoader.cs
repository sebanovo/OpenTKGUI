...
namespace OpenTKGUI
{
    class OBJLoader
    {
        ...
        {
            public List<float> VerticesArray { get; set; } = new();
            public List<uint> IndicesArray { get; set; } = new();
        }

        public static DatosOBJ ParsearDatos(string path)
        {
            string readText = File.ReadAllText(path);
            List<Vector3> vertices = new(500000);
            List<Vector2> textures = new(500000); List<Vector3> normales = new(500000);
        ...
        public static Objeto ConstruirObjeto(DatosOBJ datos, ArcRotateCamera camera, Luz luz)
        {
            Random r = new();
            Objeto newObjeto = new() { Name = r.Next().ToString() };
            Parte newParte = new Parte("Default", datos.VerticesArray, datos.IndicesArray, "C:\\Users\\HP\\Documents\\Visual Studio 2022\\Projects\\C#\\OpenTKGUI\\Resources\\Images\\ThinMatrix\\grassTexture.png", camera, luz);
            newParte.Transformation.Position = new Vector3(0.0f, 0.0f, 0.0f);
            newObjeto.Add(newParte);
            newObjeto.Transformation.Scale = new Vector3(1f / 3);
            return newObjeto;
